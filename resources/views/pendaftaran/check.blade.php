...
<main class="flex-grow">
    <section class="my-16 max-w-xl mx-auto px-4" x-data="{ showModal: {{ $errors->has('already_registered') ? 'true' : 'false' }} }">
        <h1 class="text-4xl md:text-5xl font-extrabold text-center mb-12 text-dark">Cek Pendaftaran</h1>
        ...
        <div class="bg-white p-8 rounded-lg shadow-lg">
            <form action="{{ route('pendaftaran.check') }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="nik" id="nik" value="{{ old('nik') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-dark leading-tight focus:outline-none focus:shadow-outline @error('nik') border-red-500 @enderror" required>
                    @error('nik')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-dark leading-tight focus:outline-none focus:shadow-outline @error('nama_lengkap') border-red-500 @enderror" required>
                    @error('nama_lengkap')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div x-show="showModal" @keydown.escape.window="showModal = false" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center px-4 py-6 z-50" style="display: none;">
            <div class="bg-white rounded-lg shadow-xl max-w-sm w-full p-6" @click.away="showModal = false">
