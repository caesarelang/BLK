    ...
    <div class="flex h-screen bg-gray-200">
        ...
        <div class="w-64 bg-gray-800 text-white flex flex-col">
            ...
            <nav class="flex-1 px-4 py-2">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-2 text-gray-100 hover:bg-gray-700 rounded {{ request()->routeIs('admin.dashboard') ? 'bg-gray-700' : '' }}">
                ...
                @php
                    $isKelolaDataActive = request()->routeIs('admin.programs.*') || request()->routeIs('admin.registrations.verified') || request()->routeIs('admin.faq.create');
                @endphp
                <div x-data="{ open: {{ $isKelolaDataActive ? 'true' : 'false' }} }" x-init="$watch('open', value => localStorage.setItem('sidebarOpen', JSON.stringify(value)))" class="mt-2">
                    ...
                    <div x-show="open" class="pl-8 py-2" style="display: none;">
                        <a href="{{ route('admin.programs.index') }}" class="block px-4 py-2 text-sm rounded {{ request()->routeIs('admin.programs.*') ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700' }}">Kelola Program</a>
                        <a href="{{ route('admin.registrations.verified') }}" class="block px-4 py-2 text-sm rounded {{ request()->routeIs('admin.registrations.verified') ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700' }}">Kelola Registrasi</a>
                        <a href="{{ route('admin.faq.create') }}" class="block px-4 py-2 text-sm rounded {{ request()->routeIs('admin.faq.create') ? 'bg-gray-700 text-white' : 'text-gray-400 hover:bg-gray-700' }}">Kelola FAQ</a>
                    </div>
                </div>

                <a href="{{ route('admin.registrations.verify') }}" class="flex items-center px-4 py-2 mt-2 text-gray-100 hover:bg-gray-700 rounded {{ request()->routeIs('admin.registrations.verify') ? 'bg-gray-700' : '' }}">
