...
@section('content')
    <section class="my-16 max-w-4xl mx-auto px-4" x-data="{ openModal: {} }">
        <h1 class="text-4xl md:text-5xl font-extrabold text-center mb-12 text-white">Pilihan Kejuruan</h1>

        @if($pelatihans->isEmpty())
            <p class="text-center text-gray-100 text-lg">Belum ada pelatihan yang tersedia saat ini. Silakan cek kembali nanti!</p>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($pelatihans as $pelatihan)
                    {{-- Card container with flex column layout to align items vertically --}}
                    <div class="bg-white/10 backdrop-blur-xl rounded-lg shadow-lg overflow-hidden flex flex-col text-white">
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            ...
                            <div>
                                <h3 class="font-bold text-xl mb-2 min-h-[56px]"> {{-- Added min-height for title consistency (2 lines) --}}
                                    <button @click="openModal['{{ $pelatihan->id_pelatihan }}'] = true" class="text-blue-300 hover:text-blue-200 focus:outline-none text-left">
                                        {{ $pelatihan->nama_pelatihan }}
                                    </button>
                                </h3>
                                <div class="text-sm text-gray-100 mb-2 space-y-1">
                    ...
                    <div x-show="openModal['{{ $pelatihan->id_pelatihan }}']" @keydown.escape.window="openModal['{{ $pelatihan->id_pelatihan }}'] = false" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center px-4 py-6 z-50" style="display: none;">
                        <div class="bg-white/10 backdrop-blur-xl rounded-lg shadow-xl max-w-lg w-full overflow-hidden text-white" @click.away="openModal['{{ $pelatihan->id_pelatihan }}'] = false">
                            ...
                            <div class="p-6">
                                ...
                                <div class="text-sm text-gray-100 mb-4 space-y-1">
                                    <p><strong>Deskripsi:</strong></p>
                                    <p class="text-gray-100 whitespace-pre-wrap text-base">{{ $pelatihan->deskripsi }}</p>
                                </div>
